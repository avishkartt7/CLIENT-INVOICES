...
const InvoicePreview = ({ formData = {}, client = {}, project = {}, onBack }) => {
  ...
  const calculateRows = () => {
    ...
    // Handle all deductions except Other Deduction
    ...
    // Calculate subtotal if other deduction exists
    // In the calculateRows function, update the other deduction part:
    if (formData.deductions?.otherDeduction?.checked) {
      // Calculate subtotal of all previous rows
    ...
    rows.push({
      ...
      qty: 1,
      unitPrice: subtotal.taxable,
      taxable: subtotal.taxable,
      vatPercentage: 5,
      ...
      // Add other deduction
      const otherDeductionAmount = parseFloat(formData.deductions.otherDeduction.amount) || 0;
      rows.push({
        sno: currentSno++,
        description: 'Less: Other Deductions',
        qty: 1,
        unitPrice: otherDeductionAmount,
        taxable: 'NaN',
        vatPercentage: 'NA%',
        vatAmount: 'NaN',
        totalAmount: otherDeductionAmount,
        isDeduction: true,
  ...
  const calculateTotals = (rows) => {
    if (rows.some(row => row.isOtherDeduction)) {
      // Find subtotal row
      const subtotalRow = rows.find(row => row.isSubtotal);
      // Find other deduction row
      const otherDeductionRow = rows.find(row => row.isOtherDeduction);
      
      // Get subtotal amount
      const subtotalAmount = parseFloat(subtotalRow.totalAmount);
      // Get other deduction amount
      const deductionAmount = Math.abs(parseFloat(otherDeductionRow.unitPrice));
      
      // Calculate final total by subtracting deduction from subtotal
      const finalTotal = subtotalAmount - deductionAmount;
     ...
     // Normal calculation without other deductions remains the same
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td className="border border-black p-2 text-right font-bold">
                    {totals.taxable === 'NaN' ? 'NaN' : formatNumber(totals.taxable)}
                  </td>
                  <td className="border border-black"></td>
                  <td className="border border-black p-2 text-right font-bold">
                    {totals.vatAmount === 'NaN' ? 'NaN' : formatNumber(totals.vatAmount)}
                  </td>
                  <td className="border border-black p-2 text-right font-bold">
